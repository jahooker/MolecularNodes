...
def MOL_PT_panel_local(layout_function, ):
    ...
    row_name.operator('mol.import_protein_local', 
...
def MOL_change_import_interface(
    layout_function, label: str, interface_value: int, icon: str | int
):
    ...
    op = layout_function.operator('mol.import_method_selection', **kwargs)
...
class MOL_MT_Default_Style(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        for i, label in enumerate(['Atoms', 'Cartoon', 'Ribbon', 'Ball and Stick']):
            ...
            op = layout.operator('mol.default_style',
...
def MOL_PT_panel_ui(layout_function, scene):
    ...
    grid = layout_function.box().grid_flow(columns=2)
...
def menu_item_interface(
    layout_function, label: str, node_name: str,
...
def menu_ligand_selection_custom(layout_function):
    obj = bpy.context.view_layer.objects.active
    op = layout_function.operator('mol.ligand_selection_custom',
...
class MOL_OT_Ligand_Selection_Custom(bpy.types.Operator):
    ...
    def execute(self, context):
        obj = bpy.context.view_layer.objects.active
        node_chains = nodes.chain_selection(
            node_name=f"MOL_sel_{obj.name}_ligands",
            chain_names=obj['ligands'],
            start=100,
            attribute='res_name')
        mol_add_node(node_chains.name)
        return {"FINISHED"}
...
class MOL_MT_Add_Node_Menu_Assembly(bpy.types.Menu):
    ...
    def draw(self, context):
        self.layout.operator_context = "INVOKE_DEFAULT"
        self.layout.operator("mol.assembly_bio",
                             text="Biological Assembly", emboss=True, depress=True)
        menu_item_interface(self.layout, 'Center Assembly', 'MOL_assembly_center',
...
class MOL_MT_Add_Node_Menu_Membranes(bpy.types.Menu):
    ...
    def draw(self, context):
        self.layout.operator_context = "INVOKE_DEFAULT"
        menu_item_interface(self.layout, 'Setup Atomic Properties', 'MOL_prop_setup')
...
class MOL_MT_Add_Node_Menu_Utilities(bpy.types.Menu):
    ...
    def draw(self, context):
        self.layout.operator_context = "INVOKE_DEFAULT"
        menu_item_interface(self.layout, 'Booelean Chain', 'MOL_utils_bool_chain')
        menu_item_interface(self.layout, 'Rotation Matrix', 'MOL_utils_rotation_matrix')
        menu_item_interface(self.layout, 'Curve Resample', 'MOL_utils_curve_resample')
        menu_item_interface(self.layout, 'Determine Secondary Structure', 'MOL_utils_dssp')
...
class MOL_MT_Add_Node_Menu_Density(bpy.types.Menu):
    ...
    def draw(self, context):
        self.layout.operator_context = "INVOKE_DEFAULT"
        menu_item_interface(self.layout, 'Style Surface', 'MOL_style_density_surface')
        menu_item_interface(self.layout, 'Style Wire', 'MOL_style_density_wire')
        menu_item_interface(self.layout, 'Sample Nearest Attribute', 'MOL_utils_sample_searest')
...
def mol_add_node_menu(self, context):
    if bpy.context.area.spaces[0].tree_type == 'GeometryNodeTree':
        self.layout.menu('MOL_MT_ADD_NODE_MENU', text='Molecular Nodes', icon_value=88)
